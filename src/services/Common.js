...
const commonServices = {
  ...
  dynamicInsert: async (req, table, userDetail) => {
    const db = makeDb(config);
    try {
      const capitalizedValues = Object.values(userDetail).map((value) =>
        helpers.CM.capitalizeFirstLetter(value)
      );
      const bindStr = Object.keys(userDetail).map((item) => "?");
      const sql = `INSERT INTO ${table}(${Object.keys(
        userDetail
      ).toString()}) VALUES (${bindStr.join(",")})`;
